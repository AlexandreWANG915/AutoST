...
parser = argparse.ArgumentParser(description='AutoST Training and Evaluation')

# Model detail
parser.add_argument('--model', default='AutoST', type=str, metavar='MODEL',
...
parser.add_argument('--yaml_file', default='1', type=str)
...
def main():
    ...
    yaml_file = "./Cifar10_evolution_small/config_" + args.yaml_file + ".yaml"
    with open(yaml_file, 'r') as f:
        model_cfg = yaml.safe_load(f)

    retrain_config = {'layer_num': model_cfg['RETRAIN']['DEPTH'], 'embed_dim': model_cfg['RETRAIN']['EMBED_DIM'],
    ...
    model = create_model(
        ...
        patch_size=args.patch_size, embed_dims=retrain_config['embed_dim'], num_heads=retrain_config['num_heads'],
        mlp_ratios=retrain_config['mlp_ratio'],
        in_channels=3, num_classes=args.num_classes, qkv_bias=False,
        depths=retrain_config['layer_num'], sr_ratios=1,
        T=args.time_step
    )
    print(model)
    output_dir = str(args.dataset) + '_' + str(args.time_step) + '_' + str(args.yaml_file) + '_' + str(
        args.epochs) + '_' + str(args.seed)
    ...
    try:
        for epoch in tqdm(range(start_epoch, num_epochs)):
            ...
            eval_metrics, loss, acc1 = validate(model, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast)
            ...
            if args.rank == 0:
                if max_accuracy < acc1:
                    ...
                    save_state = {'model': model_without_ddp.state_dict(),
                                  ...
                                  'epoch': epoch}
                    torch.save(save_state, os.path.join(output_dir, 'best_model.pth'))
                    print('saving...')
...
def train_one_epoch(
    ...
    return OrderedDict([('loss', losses_m.avg)]), losses_m.avg
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    ...
    metrics = OrderedDict(
        [('loss', losses_m.avg), ('top1', top1_m.avg), ('top5', top5_m.avg)]), losses_m.avg, top1_m.avg
